...
class TestULBloqueComplex(unittest.TestCase):
    ...
    def assertPlayGameEqual(self, sequence:tuple, expected_cars_positions:tuple=None, expected_return:int=0, game:dict=None):
        ...
        def mock_show_game(game, moves):
            original_show_game(game, moves)
        ...
        try:
            # Keep the original show_game function as we will patch it but wand to keep default behavior
            original_show_game = show_game
            with patch('ulbloque.getkey', side_effect=sequence) as mock_getkey, patch('ulbloque.show_game', mock_show_game):
